...
export default function Navbar({ onSearchOpen }: NavbarProps) {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full backdrop-blur-xl bg-white/80 dark:bg-slate-950/80 border-b border-slate-200/60 dark:border-slate-800/60">
      ...
      <div className="max-w-7xl mx-auto px-5 h-14 flex items-center justify-between gap-4">
        ...
        <div className="hidden md:flex items-center gap-2">
          {onSearchOpen && (
            <button
              ...
              className="flex items-center gap-2 px-2.5 py-1.5 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors text-[13px]"
              title="Search (Ctrl+K)"
            >
          ...
          <a
            ...
            className="w-8 h-8 rounded-md flex items-center justify-center text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
            aria-label="GitHub"
          >
        ...
        <div className="flex md:hidden items-center gap-1.5">
          {onSearchOpen && (
            <button onClick={onSearchOpen} className="w-8 h-8 rounded-md flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
          ...
          <button
            ...
            className="w-8 h-8 rounded-md flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
          >
            {menuOpen ? (
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-slate-100 dark:border-slate-800/60 bg-white dark:bg-slate-950 px-4 py-2 flex flex-col gap-0.5">
          {NAV_LINKS.map(link => (
